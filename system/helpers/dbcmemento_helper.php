...
if ( ! function_exists('get_location_id_by_name'))
{
	function get_location_id_by_name ($name,$type,$parent)
	{
		...
		$CI = get_instance();
		$query = $CI->db->get_where('locations',array('name'=>$name,'type'=>$type,'parent'=>$parent));
...
if ( ! function_exists('get_all_countries'))
{
	function get_all_countries()
	{
		...
		$CI->db->where(array('type'=>'country','status'=>1));
...
{
	function get_all_state($parent)
	{
		$CI = get_instance();
		$CI->load->database();
                $CI->db->select('*');
                $CI->db->from('locations');
		$CI->db->where(array('type'=>'state','status'=>1,'parent'=>$parent));
                $CI->db->order_by('name','ASC');
                $query = $CI->db->get();
...
{
	function get_all_regions($parent)
	{
		$CI = get_instance();
		$CI->load->database();
                $CI->db->select('*');
                $CI->db->from('locations');
		$CI->db->where(array('type'=>'city','status'=>1,'parent'=>$parent));
                $CI->db->order_by('name','ASC');
                $query = $CI->db->get();
                //print $CI->db->last_query();
		return $query;
