...
class Account_core extends CI_Controller {
...

	{
		...
		$this->form_validation->set_rules('useremail',  lang_key('Email'),'required|valid_email|xss_clean');
...

		{
			...
			{				
...

				{
					...
					$msg = '<div class="alert alert-danger"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.lang_key('User banned').'</strong></div>';
...

				{
					...
					$msg = '<div class="alert alert-danger"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.lang_key('Account not activated').'</strong></div>';
			...
			{				
				...
				$msg = '<div class="alert alert-danger"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.lang_key('Username or password not matched').'</strong></div>';
...

	{
		$this->session->sess_destroy();
                ...
                $this->session->unset_userdata('renew');
                $this->session->unset_userdata('list_property');
		redirect(site_url());
	}
		...
		if(get_settings('realestate_settings','enable_pricing','Yes')=='Yes')
                {
                        ...
                        $current_package_id = get_user_meta ($user_id,'current_package');
                        $package_expirtion_date = get_user_meta ($user_id,'expirtion_date');
                        $user_post_count = get_user_meta ($user_id,'post_count');

                        $my_package = $this->package_model->get_package_by_id($current_package_id);
                        $current_package_name = $my_package->title;
                        
                        $current_package_desc = $my_package->description;
                        $max_post = $my_package->max_post;

                        $value['current_package_name'] = $current_package_name;
                        $value['current_package_desc'] = $current_package_desc;
                        $value['package_expirtion_date'] = $package_expirtion_date;
                        $value['remaining_posts'] = $max_post - $user_post_count ;
                        $value['usage_posts'] = $max_post ;
                        $value['package_price'] = $my_package->price;
                        $value['package_id'] = $my_package->id;
	...
	public function renewpackage()
    {
        ...
        public function listsignup(){
            $this->session->set_userdata('list_property','1');
	...
	public function signup(){
            ...
            if($this->session->userdata('from')!='facebook') $this->session->set_userdata('from','signup');
            if(get_settings('realestate_settings','enable_pricing','Yes')=='Yes')
            {
                    $this->load->model('admin/package_model');
                    $value['packages']	= $this->package_model->get_all_packages_by_range('all');
	...
	public function terms_check($str)
	{
		...
		{
			$this->form_validation->set_message('terms_check', lang_key('You must accept terms and condition.'));
			return FALSE;
		}
		else
		{
	...
	public function useremail_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('useremail_check', lang_key('Email already in use.'));
			return FALSE;
		}
		else
		{
	...
	public function username_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('username_check', lang_key('Username already in use.'));
			return FALSE;
		}
		else
		{
	...
	public function check_email_validation($str)
	{
		...
		if ($res<=0)
		{
			$this->form_validation->set_message('check_email_validation', lang_key('Email not found.'));
			return FALSE;
		}
		else
		{
	...
	public function requestconfirmation()
	{
		...
		else
		{
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key('Confirmation email send.Please follow the instructions from that email.').'</div>');
	...
	public function confirm($email='',$code='')
	{
		...
		if($res==TRUE)
		{
			$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key('Email confirmed.You can now login with your email and password').'</div>');
			redirect(site_url('account/showmsg'));		
		}
		else
		{
			$this->session->set_flashdata('msg', '<div class="alert alert-danger">'.lang_key('Confirmation failed.Confirmation email and code doen\'t match.').'</div>');
			redirect(site_url('account/showmsg'));
		}
	}
	...
	function currentpass_check($str)
	{
		...
		if ($res<=0)
		{
			$this->form_validation->set_message('currentpass_check', lang_key('Current password Didn\'t match'));
			return FALSE;
		}
		else
		{
	...
	{
		$this->form_validation->set_rules('user_email', lang_key('Email'), 'required|valid_email|xss_clean|callback_useremail_match_check');
		...
		else
		{
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key('An email is send to your inbox.Check that email.').'</div>');
			redirect(site_url('account/showmsg'));		
		}
	}
	...
	function useremail_match_check($str)
	{
...

		{
			...
			$this->form_validation->set_message('useremail_match_check', lang_key('Email Not matched'));
...

		{
			...
			$this->form_validation->set_message('useremail_match_check', lang_key('User banned'));
...

	{
...

		{
			...
			$this->form_validation->set_message('useremail_user_ban_check', lang_key('User banned'));
...

	{
...

		{
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-block">'.lang_key('Password Recovery Link Is Not Valid').'</div>');
